...
<tr class="woocommerce-shipping-totals shipping">
	...
	<td data-title="<?php echo esc_attr( $package_name ); ?>">
		<?php if ( $available_methods ) : ?>
			<ul id="shipping_method" class="woocommerce-shipping-methods p-0">
				<?php foreach ( $available_methods as $method ) : ?>
					<li>
						...
						if ( 1 < count( $available_methods ) ) {
                            ...
                            printf( '<label class="custom-control-label" for="shipping_method_%1$s_%2$s">%3$s</label></div>', $index, esc_attr( sanitize_title( $method->id ) ), wc_cart_totals_shipping_method_label( $method ) ); // WPCS: XSS ok.
						} else {
							printf( '<input type="hidden" name="shipping_method[%1$d]" data-index="%1$d" id="shipping_method_%1$d_%2$s" value="%3$s" class="shipping_method" />', $index, esc_attr( sanitize_title( $method->id ) ), esc_attr( $method->id ) ); // WPCS: XSS ok.
                            printf( '<p>%1$s</p>', wc_cart_totals_shipping_method_label( $method ) );
						}
